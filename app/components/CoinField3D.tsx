...
function Coins({ count = 200 }) {
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      a.push([(Math.random()-0.5)*20,(Math.random()-0.3)*12,(Math.random()-0.5)*18,
  ...
  return (
    <Instances limit={count}>
      ...
      {positions.map(([x,y,z,rx,ry,rz], i) => (
        <Instance key={i} position={[x,y,z]} rotation={[rx,ry,rz]}
    ...
    <div className="absolute inset-0 hidden md:block -z-10">
      <Canvas camera={{ position: [0, 0, 12], fov: 55 }}>
        <Suspense fallback={null}>
          ...
          <EffectComposer>
            <Bloom intensity={0.6} luminanceThreshold={0.8} luminanceSmoothing={0.2} radius={0.4} />
          </EffectComposer>
        </Suspense>
      </Canvas>
    </div>
